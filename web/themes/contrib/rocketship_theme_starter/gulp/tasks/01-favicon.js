...
const generate = function(done) {
  ...
  var rePattern = new RegExp("(?<=color-spot-1:).*?(?=!default)"); // between '$color-spot-1:' and ' !default;';

  fs.readFile(config.colors, 'utf8', function (err, data) {

    if (err) {
      notify().write(err);
    }

    notify().write('find colors to use in favicons data');

    var result = data.match(rePattern);

    colorPrimary = result.toString().trim();
      ...
      realFavicon.generateFavicon({
        ...
        design: {
          ios: {
            ...
            backgroundColor: colorPrimary, //colorWhite,
            margin: '18%',
            assets: {
