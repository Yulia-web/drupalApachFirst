...
One font that is already included, is FontAwesome. It is used as an iconfont.<br />
We have the free version, if you installed this theme from Drupal.org.<br />
But if you have bought the `Pro` version, you can easily switch that by:
- replacing the font-files in your generated theme's `fonts`-folder
- change any font-file names (if needed) in `00-theme/00-base/02-fonts/_fonts.scss`
- replacing in the `fontawesome-version` variable with `pro` (instead of the default `free`) in `00-theme/00-base/00-config/00-variables/_vars-icons.scss`
